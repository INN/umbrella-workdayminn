 ...
 */
class workday_related_posts_widget extends WP_Widget {

	function __construct() {
		$widget_ops = array(
			'classname' 	=> 'workday-related-posts',
			'description' 	=> __('Lists posts related to the current post', 'largo')
		);
		parent::__construct( 'workday-related-posts-widget', __('Workday Related Posts', 'largo'), $widget_ops);
	}
	...
	function widget( $args, $instance ) {
		...
		if ( $rel_posts->have_posts() ) {
			...
			while ( $rel_posts->have_posts() ) {
				...
				echo '<a class="workday-related-post-thumbnail" href="' . get_permalink() . '">' . get_the_post_thumbnail( get_the_ID(), 'small', array( 'class' => '' ) ) . '</a>';
				...
				$excerpt = largo_excerpt(get_the_ID(), 2, false, '', true);
